	...
	implements android.widget.SeekBar.OnSeekBarChangeListener
{
	...
	{
		int i = Math.max(Math.min(Math.round(100F * mCurrentHsvValues[1]), 100), 0);
		int j = Math.max(Math.min(Math.round(100F * mCurrentHsvValues[2]), 100), 0);
	...
	private void updateSaturationGradient()
	{
		...
		af[2] = mCurrentHsvValues[2];
		int ai[] = new int[2];
		ai[0] = Color.HSVToColor(af);
		af[1] = 1.0F;
		ai[1] = Color.HSVToColor(af);
		mSaturationDetails.setGradientColors(ai);
	}
	...
	private void updateValueGradient()
	{
		...
		af[2] = 1.0F;
		ai[1] = Color.HSVToColor(af);
		mValueDetails.setGradientColors(ai);
	}
