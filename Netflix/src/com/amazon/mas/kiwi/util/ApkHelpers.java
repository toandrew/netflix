...
public class ApkHelpers
{
	...
	public static byte[] getApkSignature(String s)
	...
	private static CodeSigner[] getCodeSigners(JarFile jarfile)
	{
		...
		do
		{
			if (!enumeration.hasMoreElements())
				break;
			acodesigner = ((JarEntry)enumeration.nextElement()).getCodeSigners();
	...
	private static Certificate getFirstSigningCert(JarFile jarfile)
	{
		...
		if (acodesigner != null)
		{
			...
			if (i > 0)
			{
				List list = acodesigner[0].getSignerCertPath().getCertificates();
...
_L2:
		...
		exception1;
		throw new ApkInvalidException(exception1);
		Exception exception;
		exception;
		if (inputstream != null)
			inputstream.close();
