...
public final class ActivityResultManagerImpl_c
	implements com.amazon.android.f.ResultManager_b, b
{
	...
	public final MyActivtyResult_f a(Intent intent)
	{
		...
		a.trace((new StringBuilder()).append("Starting activity for result: ").append(intent).append(", ").append(intent.getFlags()).append(", requestId: ").append(a1.a).toString());
		d d1 = new d(this, a1);
		c.enqueueAtFront(TaskPipelineId.FOREGROUND, d1);
		MyActivtyResult_f f1;
		a.trace((new StringBuilder()).append("Blocking for request: ").append(a1.a).toString());
		f1 = (MyActivtyResult_f)g.take();
		a.trace((new StringBuilder()).append("Received Response: ").append(a1.a).toString());
		f.set(null);
		return f1;
	...
	public final boolean a(MyActivtyResult_f f1)
	{
		if (KiwiLogger.TRACE_ON)
			a.trace((new StringBuilder()).append("Recieved ActivityResult: ").append(f1).toString());
		a a1 = (a)f.get();
		if (a1 == null)
		{
			if (KiwiLogger.TRACE_ON)
		...
		{
			if (KiwiLogger.TRACE_ON)
				a.trace((new StringBuilder()).append("We don't have a request with code: ").append(f1.a).append(", returning").toString());
			return false;
		}
		if (KiwiLogger.TRACE_ON)
			a.trace((new StringBuilder()).append("Signaling thread waiting for request: ").append(f1.a).toString());
		g.add(f1);
		return true;
	}
