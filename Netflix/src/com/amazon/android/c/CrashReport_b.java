...
public final class CrashReport_b
	implements Serializable
{
	...
	public CrashReport_b(Application application, Throwable throwable)
	{
		...
		b.put("packageFilePath", application.getFilesDir().getAbsolutePath());
		...
		hashmap.put("totalInternalMemorySize", Long.toString((long)statfs.getBlockSize() * (long)statfs.getBlockCount()));
		HashMap hashmap1 = b;
		StatFs statfs1 = new StatFs(Environment.getDataDirectory().getPath());
		hashmap1.put("availableInternalMemorySize", Long.toString((long)statfs1.getBlockSize() * (long)statfs1.getAvailableBlocks()));
		...
		b.put("nativeHeapFreeSize", Long.toString(Debug.getNativeHeapAllocatedSize()));
		b.put("threadAllocCount", Long.toString(Debug.getThreadAllocCount()));
...
_L1:
		...
		{
			a.error("Error collection crash report details", throwable1);
			return;
		}
		  goto _L1
	}
	...
	{
		StringBuilder stringbuilder = new StringBuilder();
		for (Iterator iterator = Thread.getAllStackTraces().entrySet().iterator(); iterator.hasNext(); stringbuilder.append("\n\n"))
		{
			java.util.Map.Entry entry = (java.util.Map.Entry)iterator.next();
			Thread thread = (Thread)entry.getKey();
			StackTraceElement astacktraceelement[] = (StackTraceElement[])entry.getValue();
			stringbuilder.append((new StringBuilder()).append("Thread : ").append(thread.getId()).toString());
	...
	private void c()
	{
		...
		try
		{
			...
			stringbuilder.append((String)b.get("packageName")).append((String)b.get("packageVersionName")).append((String)b.get("androidVersion"));
		...
		for (matcher = Pattern.compile("([a-zA-Z0-9_.]+(Exception|Error))|(at\\s.*\\(.*\\))").matcher(s); matcher.find(); stringbuilder.append(matcher.group()));
		String s1 = (new BigInteger(MessageDigest.getInstance("SHA1").digest(stringbuilder.toString().getBytes("UTF-8")))).abs().toString(16);
		b.put("crashId", s1);
		return;
	}
