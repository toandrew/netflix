...
public final class DataAuthenticationKeyLoader_c {
        ...
        try {
            if (KiwiLogger.TRACE_ON)
                a.trace((new StringBuilder())
                        .append("Extracting cert from entry: ")
                        .append(jarentry.getName()).toString());
            CertificateFactory certificatefactory = CertificateFactory
                    .getInstance("X.509");
            if (KiwiLogger.TRACE_ON)
                a.trace("Generating certificates from entry input stream");
            certpath = certificatefactory.generateCertPath(new ArrayList(
                    certificatefactory.generateCertificates(jarfile
                            .getInputStream(jarentry))));
        } catch (Exception exception) {
            throw com.amazon.android.h.CertFailedToLoadKiwiException_a
                    .a(exception);
        }
        return certpath;
    }
    ...
    private JarFile b() throws CertFailedToLoadKiwiException_a {
        String s = b.getPackageCodePath();
        if (KiwiLogger.TRACE_ON)
            a.trace((new StringBuilder()).append("Opening apk: ").append(s)
                    .toString());
        JarFile jarfile;
        try {
    ...
    private static CertVerifier_b c() throws CertFailedToLoadKiwiException_a {
        CertVerifier_b b1;
        try {
            b1 = new CertVerifier_b();
        } catch (GeneralSecurityException generalsecurityexception) {
            throw new CertFailedToLoadKiwiException_a(
                    "FAILED_TO_ESTABLISH_TRUST", generalsecurityexception);
        }
        return b1;
    }

    public final PublicKey a() throws CertFailedToLoadKiwiException_a {
        CertPath certpath;
        if (KiwiLogger.TRACE_ON)
        ...
        if (certpath == null || certpath.getCertificates().size() <= 0) {
            ...
            Certificate certificate = (Certificate) certpath.getCertificates()
                    .get(0);
            if (!(certificate instanceof X509Certificate)) {
                flag = false;
                if (!flag)
                    throw new CertFailedToLoadKiwiException_a("CERT_INVALID",
                            null);
            }
        }

        if (!c().a(certpath))
            throw new CertFailedToLoadKiwiException_a("VERIFICATION_FAILED",
                    null);
        PublicKey publickey1 = ((X509Certificate) certpath.getCertificates()
                .get(0)).getPublicKey();
        if (KiwiLogger.TRACE_ON)
