...
public final class ForegroundTaskPipeline_e implements b, f {
    ...
    public ForegroundTaskPipeline_e(f f1) {
        f = new ArrayList();
        d = com.amazon.android.framework.task.pipeline.SimpleTaskPipeline_a
                .b("KIWI_UI");
        e = f1;
    }
    ...
    private void a(Task task, boolean flag) {
        ...
        if (KiwiLogger.TRACE_ON)
            a.trace((new StringBuilder())
                    .append("No UI visible to execute task: ").append(task)
                    .append(", placing into pending queue until task ")
                    .append("is visible").toString());
        f.add(task);
    }

    static void a(ForegroundTaskPipeline_e e1) {
        if (KiwiLogger.TRACE_ON)
            a.trace("Activity resumed, scheduling tasks on UI thread");
        for (Iterator iterator = e1.f.iterator(); iterator.hasNext(); e1.a(
                (Task) iterator.next(), true))
            ;
        e1.f.clear();
    }

    static void a(ForegroundTaskPipeline_e e1, Task task) {
        e1.a(task, true);
    }

    private Task c(Task task) {
        return new PostToUITask_c(this, task);
    }
