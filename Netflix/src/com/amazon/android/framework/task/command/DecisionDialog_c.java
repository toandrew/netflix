...
import com.amazon.android.framework.prompt.d;
...
public final class DecisionDialog_c extends Prompt
{
	...
	private static ActivityInfo a(Activity activity)
	{
		...
		catch (android.content.pm.PackageManager.NameNotFoundException namenotfoundexception)
		{
			if (KiwiLogger.ERROR_ON)
				a.error((new StringBuilder()).append("Unable to get info for activity: ").append(activity).toString());
			return null;
		}
		return activityinfo;
	}
	...
	private void a(AlertDialog alertdialog, l l1, int i)
	{
		...
		{
			alertdialog.setButton(i, l1.b, new com.amazon.android.framework.task.command.d(this, l1));
			return;
		}
	}
	...
	private n c()
		throws com.amazon.android.ResultManager_b.ResourceManagerImpl_c
	{
		n n;
		try
		{
			if (KiwiLogger.TRACE_ON)
				a.trace((new StringBuilder()).append("Blocking while awaiting customer decision: ").append(Thread.currentThread()).toString());
			n = ((l)d.take()).a;
		}
		catch (InterruptedException interruptedexception)
		{
			...
			a a1;
			if (getExpirationReason() == d.a)
				a1 = a.b;
			else
				a1 = a.a;
			throw new com.amazon.android.ResourceManagerImpl_c.c(a1);
		}
		return n;
	}
	...
	{
		if (KiwiLogger.TRACE_ON)
			a.trace((new StringBuilder()).append("GetCustomerDecision: ").append(b).toString());
		return c();
	}
	...
	public final boolean doCompatibilityCheck(Activity activity)
	{
		...
		if (activityinfo.launchMode == 3)
			...
			flag1 = false;
		a.trace((new StringBuilder()).append("Single instance: ").append(flag1).toString());
		if ((2 & activityinfo.flags) != 0)
			flag2 = true;
		else
			flag2 = false;
		a.trace((new StringBuilder()).append("Finish on task launch:").append(flag2).toString());
		if ((0x80 & activityinfo.flags) != 0)
			flag3 = true;
		else
			flag3 = false;
		a.trace((new StringBuilder()).append("No History: ").append(flag3).toString());
	...
	protected final void doExpiration(d d1)
	{
		if (KiwiLogger.TRACE_ON)
			a.trace((new StringBuilder()).append("Expiring Decision Dialog: Thread: ").append(Thread.currentThread()).toString());
		b.interrupt();
	}
	...
	{
		return (new StringBuilder()).append("DecisionDialog: ").append(c.a).toString();
	}
