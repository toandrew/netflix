...
public abstract class AbstractCommandTask extends c {
    ...
    private static final KiwiLogger LOGGER = new KiwiLogger(
            "AbstractCommandTask");
    ...
    private void expire(r r,
            com.amazon.android.framework.task.command.FailedReason_a a1)
            throws RemoteException, KiwiException {
        if (KiwiLogger.TRACE_ON)
            LOGGER.trace((new StringBuilder()).append("Expiring Decision: ")
                    .append(r).append(", reason: ").append(a1).toString());
    ...
    private Metric_b extractMetric(KiwiException kiwiexception) {
        ...
        b1.a("subType", kiwiexception.getType())
                .a("reason", kiwiexception.getReason())
                .a("context", kiwiexception.getContext());
        return b1;
    }
    ...
    private String getFailureMetricName() {
        return (new StringBuilder()).append(getCommandName())
                .append("_failure").toString();
    }

    private void handleChoice(r r, n n1) throws RemoteException, KiwiException {
        if (KiwiLogger.TRACE_ON)
            LOGGER.trace((new StringBuilder())
                    .append("Handling customer choice: ").append(n1).toString());
        ...
        if (intent != null) {
            ...
            if (f1 == null) {
                ...
                expire(r,
                        com.amazon.android.framework.task.command.FailedReason_a.EXPIRATION_DURATION_ELAPSED_a);
                return;
            }
            if (f1.b == 0) {
                if (KiwiLogger.TRACE_ON)
                ...
                expire(r,
                        com.amazon.android.framework.task.command.FailedReason_a.ACTION_CANCELED_c);
                return;
            }
            if (KiwiLogger.TRACE_ON)
    ...
    private void handleCommandResult(CommandResult_k k1) throws KiwiException,
            RemoteException {
        if (KiwiLogger.TRACE_ON)
            LOGGER.trace((new StringBuilder())
                    .append("Received result from CommandService: ").append(k1)
                    .toString());
        if (k1 == null) {
            if (KiwiLogger.TRACE_ON)
    ...
    private void handleDecision(r r) throws RemoteException, KiwiException {
        if (KiwiLogger.TRACE_ON)
            LOGGER.trace("Handling Decision");
        n n1;
        try {
            com.amazon.android.framework.task.command.DecisionDialog_c c1 = new com.amazon.android.framework.task.command.DecisionDialog_c(
                    r);
    ...
    private void handleException(j j) throws RemoteException, KiwiException {
    ...
    private void handleExecutionException(Throwable throwable) {
        if (KiwiLogger.TRACE_ON)
            LOGGER.trace(
                    (new StringBuilder())
                            .append("Exception occurred while processing task: ")
    ...
    private void handleFailure(FailureResult failureresult)
        ...
        if (KiwiLogger.TRACE_ON)
            LOGGER.trace((new StringBuilder())
                    .append("Command failed execution: ")
                    .append(failureresult.getDisplayableName()).toString());
    ...
    private KiwiException translate(Throwable throwable) {
        ...
        if (throwable instanceof RemoteException)
            return new CommandServiceKiwiException_a(
    ...
    public final void execute() {
        if (KiwiLogger.TRACE_ON) {
            LOGGER.trace("----------------------------------------------");
            LOGGER.trace((new StringBuilder()).append("Executing: ")
                    .append(getCommandName()).toString());
            LOGGER.trace("----------------------------------------------");
        }
        try {
            preExecution();
            ...
            if (KiwiLogger.TRACE_ON)
                LOGGER.trace((new StringBuilder())
                        .append("Executing Command: ").append(getCommandName())
                        .toString());
    ...
    protected void onException(KiwiException kiwiexception) {
        LOGGER.error((new StringBuilder()).append("On Exception!!!!: ")
                .append(kiwiexception).toString());
    }
